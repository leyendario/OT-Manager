...
namespace FastColoredTextBoxNS
{
    ...
    public class Range : IEnumerable<Place>
    {
        ...
        public void GoLeft(bool shift)
        {
            if (start.iChar != 0 || start.iLine != 0)
            {
                if (start.iChar > 0 && tb.lines[start.iLine].VisibleState == VisibleState.Visible)
                    start.Offset(-1, 0);
                else
                {
                    int i = tb.FindPrevVisibleLine(start.iLine);
                    if (i == start.iLine) return;
                    start = new Place(tb.lines[i].Count, i);
                }
            }
        ...
        public void GoRight(bool shift)
        {
            if (start.iLine < tb.lines.Count - 1 || start.iChar < tb.lines[tb.lines.Count - 1].Count)
            {
                if (start.iChar < tb.lines[start.iLine].Count && tb.lines[start.iLine].VisibleState == VisibleState.Visible)
                    start.Offset(1, 0);
                else
                {
        ...
        public void SetFoldingMarkers(string startFoldingPattern, string finishFoldingPattern, RegexOptions options)
        {
            foreach (var range in GetRanges(startFoldingPattern, options))
                tb.lines[range.Start.iLine].FoldingStartMarker = startFoldingPattern;

            foreach (var range in GetRanges(finishFoldingPattern, options))
                tb.lines[range.Start.iLine].FoldingEndMarker = startFoldingPattern;
            //
            tb.Invalidate();
        }
        /// <summary>
        ...
        public void ClearFoldingMarkers()
        {
            ...
            //
            for (int y = fromLine; y <= toLine; y++)
                tb.lines[y].ClearFoldingMarkers();
            //
            tb.Invalidate();
        }
            ...
            do
            {
                ...
                switch (range.CharAfterStart)
                {
                    ...
                    case '\n':
                        ...
                        break;
                    case ' ':
                        if (wasNonSpace) wasSpace = true;
                        break;
                    default:
                        if (wasSpace && wasNonSpace) goto found;
                        wasNonSpace = true; break;
                }
                //move right
                range.GoRight(shift);
